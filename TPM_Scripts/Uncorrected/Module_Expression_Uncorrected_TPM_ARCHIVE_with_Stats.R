...
EnrichmentDF <-
  fread("/Users/a703402454/Desktop/UCSC_Project_Testing/Test_Data_Set_70%/TPM_Analysis/WGCNA_Plots_Uncorrected/Module_Enrichment_Uncorrected.csv",
        sep = ",",
        header = TRUE,
        data.table = FALSE
        )
...
getwd() %>%
  paste0("/Cluster_Composition_Uncorrected.csv") %>%
  fwrite(clusterTable,
         .,
         sep = ",",
         row.names = TRUE,
...
png("Tissue-Wise_Expression_of_Modules_Uncorrected.png",
    height = 27,
    width = 95,
    units = "cm",
    res = 400
...
ggplot(ModulePlotDF,
  ...
  theme(strip.text.x = element_text(size = 25,
                                    face = "bold",
                                    margin = margin(t = 0.35, b = 0.35, unit = "cm")
                                    ),
  ...
  labs(y = "Tumor Expression (TPM + 1, Log scale)",
       fill = "Tissue",
       color = "Tissue") +
  facet_wrap(~Module,
             nrow = 1,
...
png("Cluster-Wise_Expression_of_Modules_Uncorrected.png",
    height = 27,
    width = 95,
    units = "cm",
    res = 400
